...
export default function Tarif({ jenisLayanan, jenisPasien, existingTarif }) {
  ...
  const activeTarif = useMemo(() => {
    return (tarifList || []).filter((t) => t.aktif === true || t.aktif === 1);
  }, [tarifList]);

  const inactiveTarif = useMemo(() => {
    return (tarifList || []).filter((t) => t.aktif === false || t.aktif === 0);
  }, [tarifList]);
  ...
  return (
    <LabkesdaLayout>
      <Head title={`Tarif Aktif: ${jenisLayanan?.nama ?? 'Jenis Layanan'}`} />
                ...
                {activeTarif.length > 0 ? (
                  activeTarif.map((row) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-2">{formatRp(row.harga)}</td>
                      <td className="px-4 py-2">
                        {new Date(row.valid_dari).toLocaleDateString() || '—'}
                      </td>
                      <td className="px-4 py-2">
                        {row.valid_sampai
                          ? new Date(row.valid_sampai).toLocaleDateString()
                          : '—'}
                      </td>
                ...
                <tbody>
                  {inactiveTarif.length > 0 ? (
                    inactiveTarif.map((row) => (
                      <tr
                        key={row.id ?? row.jenis_pasien?.kode}
                        className="border-b hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-700"
                      >
                        <td className="text-nowrap px-4 py-2">
                          {row.jenis_pasien?.nama}
