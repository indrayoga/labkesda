...
import CreateDokterForm from './CreateDokterForm';

export default function Index({ dokter }) {
  ...
  const handleFilter = () => {
    router.visit(route('dokter.index'), {
      method: 'get',
      data: {
        nama: cariNama,
      },
  ...
  const submit = (e) => {
    e.preventDefault();
    if (selectedDokter) {
      put(route('dokter.update', selectedDokter.id), {
        onSuccess: () => {
  ...
  const hapusDokter = (dokter) => {
    router.delete(route('dokter.destroy', dokter.id), {
      onSuccess: () => {
        ...
        <div className="relative overflow-hidden bg-white shadow-md dark:bg-gray-800">
          <div className="flex flex-col space-y-3 px-4 py-3 lg:flex-row lg:items-center lg:justify-between lg:space-x-4 lg:space-y-0">
            <div className="flex flex-1 items-center space-x-4">
              <h2>Daftar Dokter ({dokter.total} entri)</h2>
            </div>
        ...
        <div className="relative overflow-hidden bg-white shadow-md sm:rounded-b-lg dark:bg-gray-800">
          ...
          <div className="flex items-center justify-between border-t bg-gray-50 px-4 py-3 dark:border-gray-600 dark:bg-gray-700">
            <span className="text-sm text-gray-700 dark:text-gray-400">
              Menampilkan <span className="font-semibold">{dokter.from}</span>{' '}
              sampai <span className="font-semibold">{dokter.to}</span> dari
              total <span className="font-semibold">{dokter.total}</span> entri
            </span>
            <div className="xs:mt-0 mt-2 inline-flex">
              {dokter.links.map((link, index) => (
                <Link
                  href={link.url || '#'}
                  key={index}
        ...
        <ModalBody>
          <div className="max-w-md text-center">
            ...
            <div className="flex justify-center gap-4">
              <Button onClick={() => hapusDokter(selectedDokter)}>
                Ya, Saya yakin
              </Button>
              <Button
                color="alternative"
